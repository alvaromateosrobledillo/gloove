...
const ResponsiveSection: React.FC = () => {
  ...
  useEffect(() => {
    const updateVideoSrc = () => {
      ...
      if (width >= 1920) {
        setVideoSrc("/public/colaboradores/RuletaO2.mp4"); // Monitores grandes
        setIsLargeScreen(true);
      } else if (width >= 1024) {
        setVideoSrc("/public/colaboradores/RuletaO2.mp4"); // PC
        setIsLargeScreen(true);
      } else if (width >= 768) {
        setVideoSrc("/public/colaboradores/RuletaT.mp4"); // Tablets
        setIsLargeScreen(false);
      } else {
        setVideoSrc("/public/colaboradores/RuletaM.mp4"); // Móviles
        setIsLargeScreen(false);
      }
    };
